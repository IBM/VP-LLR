...
class CLIP_encode_img(nn.Module):
    def __init__(self, encode_image):
        super(CLIP_encode_img, self).__init__()
        self.encode_image = encode_image
    def forward(self, x):
        x_emb = self.encode_image(x)
        return x_emb
        
class CLIP_network_module(nn.Module):
    def __init__(self, txt_emb, encode_image, logit_scale):
        super(CLIP_network_module, self).__init__()
    ...
    def forward(self, x):
        ...
        logits = self.logit_scale.exp() * x_emb @ self.txt_emb.t()
        return logits
