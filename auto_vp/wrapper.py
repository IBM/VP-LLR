...
class BaseWrapper(nn.Module):
    def __init__(self, model_name=None, dataset_name=None, input_perturbation=None, output_mapping=None, train_resize=None, init_scale=1.0, clip_img_size=128, device=None):
        ...
        if((self.model_name == "clip") and self.output_mapping.mapping_method == "fully_connected_layer_mapping" and self.output_mapping.weightinit == True):
        ...
        self.model_zoo = ["resnet18", "ig_resnext101_32x8d", "vit_b_16", "clip", "swin_t"]
    ...
    def forward(self, input):
        ...
        if(self.model_name[0:4] == "clip"):
            # x = self.CLIP_network(x) # for training
            x = self.CLIP_network_module(x) # for evidence computation
        else:
            x = self.model(x)
